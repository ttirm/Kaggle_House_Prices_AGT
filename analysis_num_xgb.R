...
xgb_tune <-train(SalePrice~PC1+PC2+PC3+PC4+PC5,
                 data=train.pca,
                 method="xgbTree",
                 metric = "RMSE",
                 trControl=cv.ctrl, 
...
train_Data <- train.pca[, c("SalePrice","PC1","PC2","PC3","PC4","PC5")]
xgb_train <- xgb.DMatrix(model.matrix(~ PC1+PC2+PC3+PC4+PC5, data = train.pca),
                         label=y_train, missing=NA)
...
write.csv(Submit, "./predictions/submission_pca_num_5.csv", row.names=FALSE)
